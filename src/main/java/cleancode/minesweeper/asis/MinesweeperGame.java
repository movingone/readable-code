...
//                e.printStackTrace();
                ...
                System.out.println("프로그램에 예기치 않은 문제가 생겼습니다.");
            }
        }
    }

    private static void actOnCell(String cellInput, String userActionInput) {
        int selectColIndex = getSelectColIndex(cellInput);
        int selectRowIndex = getSelectRowIndex(cellInput);

        if (doesUserChooseToPlantFlag(userActionInput)) {
            BOARD[selectRowIndex][selectColIndex] = Cell.of(FLAG_SIGN);
            checkIfGameIsOver();
            return;
        }

        if (doesUserChooseToOpenCell(userActionInput)) {
            if (isLandMineCell(selectRowIndex, selectColIndex)) {
                BOARD[selectRowIndex][selectColIndex] = Cell.of(LAND_MINE_SIGN);
                changeGamesStatusToLose();
                return;
            }

            open(selectRowIndex, selectColIndex);
            checkIfGameIsOver();
            return;
        }
    ...
    private static void changeGamesStatusToLose() {
        gameStatus = -1;
    }

    private static boolean isLandMineCell(int selectRowIndex, int selectColIndex) {
        return LAND_MINES[selectRowIndex][selectColIndex];
    }
    ...
    private static int getSelectColIndex(String cellInput) {
    ...
    private static void changeGamesStatusToWin() {
        gameStatus = 1;
    }
    ...
    private static int convertRowFrom(char cellInputRow) {
        ...
        if (rowIndex >= BOARD_ROW_SIZE)
            throw new GameException("8 보다 큰 Index 입니다.");
    ...
    private static void open(int row, int col) {
        ...
        if (NEARBY_LAND_MINE_COUNTS[row][col] != 0) {
            BOARD[row][col] = Cell.of(String.valueOf(NEARBY_LAND_MINE_COUNTS));
            return;
        } else {
